...
describe("POST /api/admin/auth/login", () => {
  ...
  it("returns 400 when password is missing", async () => {
    const res = await request(app).post("/api/admin/auth/login").send({ email: "user@example.com" });
    expect(res.status).toBe(400);
  });
...
describe("POST /api/admin/auth/forgot-password/send-otp", () => {
  it("returns 400 when email is missing", async () => {
    const res = await request(app).post("/api/admin/auth/forgot-password/send-otp").send({});
    expect(res.status).toBe(400);
  });

  it("returns 400 for invalid email format", async () => {
    const res = await request(app).post("/api/admin/auth/forgot-password/send-otp").send({ email: "not-an-email" });
    expect(res.status).toBe(400);
  });
});
...
describe("POST /api/admin/auth/forgot-password/verify-otp", () => {
  it("returns 400 when all fields are missing", async () => {
    const res = await request(app).post("/api/admin/auth/forgot-password/verify-otp").send({});
    expect(res.status).toBe(400);
  });
