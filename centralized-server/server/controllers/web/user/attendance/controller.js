...
async function recordAttendance(faceRes, user, userId, res) {
  ...
  if (!attendance) {
    ...
    const activeMembership = await ProjectMember.findOne({ userId, isActive: true }).sort({ assignedDate: -1 }).lean();
  ...
  if (locationLockRaw) {
    try {
      ...
      if (user.organizationId) {
        ...
        const serverIp =
          (req.headers["x-forwarded-for"] || "").split(",")[0].trim() || req.headers["x-real-ip"] || req.ip || null;
