...
/**
 * WhatsApp sends wa_id with country code (e.g. 917815873262 or +917815873262).
...
export async function buildContext(internalMessage, intentResult) {
  ...
  if (employee?.userId) {
    ...
    if (LEAVE_INTENTS.has(intent)) {
      const [balance, recent] = await Promise.all([getLeaveBalance(employee.userId), getRecentLeaves(employee.userId)]);
